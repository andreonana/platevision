...
\begin{table}[H]
  ...
  \begin{tabular}{lr}
    ...
    Rappel macro            & 3{,}61\,\% \\
    Top-3 accuracy          & 8{,}54\,\% \\
    Temps d'inférence moy.  & \todo{mesure post-entraînement complet} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  \IfFileExists{../figures/yolo_training_curves.png}{%
    ...
    \fbox{\parbox{0.9\textwidth}{\centering\vspace{2cm}%
      \todo{Figure yolo\_training\_curves.png — générer après entraînement :\\
...
\begin{table}[H]
  ...
  \begin{tabular}{lrl}
    ...
    \midrule
    mAP@0.5           & \todo{yolo\_metrics.json} & Détection plaque \\
    mAP@0.5:0.95      & \todo{yolo\_metrics.json} & Localisation précise \\
    Précision YOLO    & \todo{yolo\_metrics.json} & TP / (TP+FP) \\
    Rappel YOLO       & \todo{yolo\_metrics.json} & TP / (TP+FN) \\
    CER moyen         & \todo{yolo\_ocr\_evaluation.json} & Erreur caractère \\
    WER moyen         & \todo{yolo\_ocr\_evaluation.json} & Erreur plaque entière \\
    Lecture parfaite  & \todo{yolo\_ocr\_evaluation.json} & Plaques sans erreur \\
    Inférence totale  & \todo{yolo\_ocr\_evaluation.json} & Seuil MINT : $<$100~ms \\
    \bottomrule
  \end{tabular}
\end{table}

\medskip
\textit{Note} : les valeurs marquées \todo{} seront renseignées après
exécution de \texttt{python main.py --module A2 --train --epochs 50}
puis \texttt{python main.py --module A2 --evaluate}.
...
\begin{figure}[H]
  ...
  \IfFileExists{../figures/comparative_figure.png}{%
    ...
    \fbox{\parbox{0.9\textwidth}{\centering\vspace{2cm}%
      \todo{Figure comparative\_figure.png — générer après entraînement :\\
...
\begin{table}[H]
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    Accuracy / Perfect Read  & 2{,}75\,\%  & \todo{} & — \\
    F1-macro / (1–CER)       & 1{,}20\,\%  & \todo{} & — \\
    Top-3 / Detection Rate   & 8{,}54\,\%  & \todo{} & — \\
    CER moyen                & N/A         & \todo{} & — \\
    WER moyen                & N/A         & \todo{} & — \\
    mAP@0.5                  & N/A         & \todo{} & — \\
    mAP@0.5:0.95             & N/A         & \todo{} & — \\
    Temps inférence (ms)     & $\sim$1~ms  & \todo{} & \todo{×N} \\
    Temps réel MINT ($<$100~ms) & Oui      & \todo{} & — \\
    \midrule
...
Le pipeline YOLO+OCR atteint un mAP@0.5 de \todo{valeur} et lit les plaques
avec un CER moyen de \todo{valeur}. Ces performances permettent une lecture
fiable en conditions normales. Néanmoins, le système produit une liste de
