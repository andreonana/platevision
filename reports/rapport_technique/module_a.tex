...
\begin{figure}[H]
  ...
  \IfFileExists{figures/yolo_training_curves.png}{%
    ...
    \fbox{\parbox{0.9\textwidth}{\centering\vspace{2cm}%
      \textit{Figure yolo\_training\_curves.png --- disponible apr\`{e}s entra\^{i}nement :}\\[4pt]
...
\begin{table}[H]
  ...
  \begin{tabular}{lrl}
    ...
    mAP@0.5:0.95      & \textbf{99{,}50\,\%} & Localisation pr\'{e}cise \\
    Pr\'{e}cision YOLO & $\approx$\textbf{100\,\%} & TP / (TP+FP) \\
    ...
    Inf\'{e}rence totale & \textbf{8{,}0\,ms} & $\ll$ seuil MINT 100~ms~(GPU RTX~3050) \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  \IfFileExists{figures/comparative_figure.png}{%
    ...
    \fbox{\parbox{0.9\textwidth}{\centering\vspace{2cm}%
      \textit{Génération :} \texttt{python main.py --module A --compare}%
      \vspace{2cm}}}%
  }
...
\begin{table}[H]
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    Accuracy~/~Perfect Read$^\dagger$  & 27{,}82\,\% & 0{,}00\,\%  & T\^{a}ches diff. \\
    F1-macro~/~(1$-$CER)$^\dagger$    & 24{,}81\,\% & $-$2\,\%    & Baseline OCR \\
    Top-3~/~D\'{e}tection Rate        & 41{,}60\,\% & \textbf{100\,\%} & $+$58{,}4~pts \\
    ...
    mAP@0.5:0.95                      & N/A         & \textbf{99{,}50\,\%} & Localisation pr\'{e}cise \\
    Temps inf\'{e}rence (ms)          & $\sim$1~ms  & \textbf{8{,}0~ms} & $\times$8 (GPU RTX~3050) \\
...
d\'{e}tection des plaques (inf\'{e}rence 8{,}0~ms, GPU RTX~3050).
